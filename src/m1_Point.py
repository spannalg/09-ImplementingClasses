...
import math
class Point(object):
    """ Holds data for a 2d point and their operations"""
    def __init__(self, x, y):
        """Init holds the instance variable"""
        self.x = x
        self.y = y
        self.moves_made = 0
        self.distance = 0
    ...
    def get_distance_from(self, p1):
        total_x = p1.x - self.x
        total_y = p1.y - self.y
        hypotenuse = ((total_x ** 2) + (total_y ** 2)) ** .5
        return hypotenuse
    ...
    def move_to(self, x, y):
        ...
        self.distance_moved += self.get_distance_from(Point(self.x_initial,self.y_initial))



    def move_by (self, x, y):
        self.distance = self.distance + self.get_distance_from(Point(self.x+x,self.y+y))
        x_start = self.x
        y_start = self.y
        self.x = self.x + x
        self.y = self.y + y
        self.moves_made = self.moves_made + 1
        self.distance_moved += self.get_distance_from(Point(x_start,y_start))

    def get_number_of_moves_made(self):
        return self.moves_made

    def get_distance_from_start(self):
        delta_x = self.x - self.x_start
        delta_y = self.y - self.y_start
        hypotenuse = math.sqrt((delta_x**2)+(delta_y**2))
        return hypotenuse
    def get_distance_traveled(self):
        return self.distance_moved
    def closer_to(self,p1,p2):
        delta_x_1 = p1.x - self.y
        delta_y_1 = p1.y - self.y
        delta_x_2 = p2.x - self.x
        delta_y_2 = p2.y - self.y
        hypotenuse_1 = math.sqrt((delta_x_1**2)+(delta_y_1**2))
        hypotenuse_2 = math.sqrt((delta_x_2 ** 2) + (delta_y_2 ** 2))
        if hypotenuse_1 > hypotenuse_2:
            return p1
        else:
            return p2
    def halfway_to(self,p1):
        midpoint_x = (self.x+p1.x)/2
        midpoint_y = (self.y+p1.y)/2
